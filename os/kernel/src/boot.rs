...
pub extern "C" fn start(multiboot2_magic: u32, multiboot2_addr: *const BootInformationHeader) {
    ...
    */
    ...
    info!(
        "Total free frames #{}, memory::frames::get_total_free_frames()",
        memory::frames::get_total_free_frames()
    );
    ...
    // Start RPC server at boot (userspace process). Place your server binary
    // at `initrd/usr/bin/rpc_server` or similar and use the same path here.
    scheduler().ready(Thread::load_application("bin/rpc_server", "rpc_server", &[].to_vec()).expect("failed to load rpc_server"));
